...
describe('VerifySignature', function () {
	...
	const recoverSignature = async (
		...
		const recoveredAddress = ethers.utils.verifyTypedData(domain, types, buy, signature);
	...
	it('Signer1 signs the buy object of signer2 which must revert.', async function () {
		...
		const { id, timestamp, buyer, nonce, signature } = result;
		const recoveredAddress = await recoverSignature(
			signatureVerifier.address,
			signature,
			buyer,
			timestamp,
		...
		await expect(
			...
			.withArgs(recoveredAddress, buyer);
	});
	it('A valid signature from signer1 can go through the verification.', async function () {
		const buyerNonce = await signatureVerifier.getNonce(signer2.address);
		const result = await setupSignature(
	...
	it('A valid signature must revert if nonce is not correct ', async function () {
		const buyerNonce = await signatureVerifier.getNonce(signer2.address);
		// mine a new block with timestamp `newTimestamp`
		const result = await setupSignature(
			signatureVerifier.address,
		...
		await expect(
			...
			.to.revertedWithCustomError(signatureVerifier, 'InvalidNonce')
			.withArgs(nonce, buyerNonce);
	});
});
